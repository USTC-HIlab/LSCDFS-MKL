...
clf
hold on
S1=stairs(t1,T1,'b'); %Kaplan-Meier curve for Long time survivor
if ~isempty(table12)
    S3=plot(xcg1,ycg1,'k+'); %Censored data for Long time survivor (if there are)
else
    S3=[];
end

S2=stairs(t2,T2,'r'); %Kaplan-Meier curve for Short time survivor
if ~isempty(table22)
    S3=plot(xcg2,ycg2,'k+'); %Censored data for Short time survivor (if there are)
end
hold off
...
xlabel('Time to recurrence [months]')
if isempty(S3)
    legend([S1 S2],'Longer time survivor','Shorter time survivor')
else
    legend([S1 S2 S3],'Longer time survivor','Shorter time survivor','Censored')
end
hold on
...
c=find(table(:,3)==0); %find the "pigeon-holes" of Long time survivor
for I=1:length(c)
    if c(I)~=1
...
%Compute the difference between observed deaths for Long time survivor and
...
annotation('textbox',[.61 .7 .02 .02],'String',strcat('p=',num2str(p)),'FitBoxToText','on');
hold off
...
fprintf('Longer time survivor: Hazard rate: %0.4f\n',lambda1)
fprintf('Shorter time survivor: Hazard rate: %0.4f\n',lambda2)
